...
def usage():
    print(
        'Usage:\n\t%s -i [input_filename] -o [output_filename]' %
        sys.argv[0])
    print('Optional:')
...
def add_watermark_text(im, text, angle=23, opacity=0.25,
...
def main(argv):
    ...
    try:
        opts, args = getopt.gnu_getopt(
            argv, "hi:o:f:w:p:c:t:a:s:x:y:m:", [
                "help", "input=", "output=", "format=", "watermark_image", "position=", "opacity=", "text=", "angle=", "show", "position_x=", "position_y=", "mode="])
    ...
    if None != watermark_filename:
        ...
        im_marked = add_watermark_image(
            im, mark, position_x, position_y, opacity)
        if show_flag:
            im_marked.show()

    # text watermark
    if None != text:
        im_marked = add_watermark_text(
            im, text, angle, opacity, font='/Library/Fonts/Copperplate.ttc')
